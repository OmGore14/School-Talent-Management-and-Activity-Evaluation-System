...
#Defining the load participant funciton.
def load_participant_data(filepath: str) -> tuple[list[Student], list[Teacher]]:
    students = []
    teachers = []
    # Reading the parameter as per instrucutions and initializing the objects.
...
# Defining the load activities function
def load_activities_data(filepath: str) -> tuple[list[SportsTournament], list[TalentShow], list[AcademicCompetition]]:
    sports_list = []
    talent_list = []
    academic_list = []
      # Reading the parameter as per instrucutions and initializing the objects.
    with open(filepath, 'r') as file:
        ...
        for row in reader:
            ...
            if type_ == "Sports":
                ...
                obj = SportsTournament(activity_id, name, type_, max_participants, grade_level, False, [], [], game_type, duration)
                sports_list.append(obj)

            elif type_ == "Talent":
                talents = [t.strip() for t in row['talent_categories'].split(',')] if row['talent_categories'] else []
                obj = TalentShow(activity_id, name, type_, max_participants, grade_level, False, [], [], talents)
                talent_list.append(obj)

            elif type_ == "Academic":
                subjects = [s.strip() for s in row['subjects'].split(',')] if row['subjects'] else []
                max_marks = float(row['max_marks'])
                obj = AcademicCompetition(activity_id, name, type_, max_participants, grade_level, False, [], [], subjects, max_marks)
                academic_list.append(obj)

    return sports_list, talent_list, academic_list
...
def register_activity(participant_filepath: str, activity_filepath: str) -> Tuple[List, List, List, List, List, List]:
    participants, teachers = load_participant_data(participant_filepath)
    sports, talent, academics = load_activities_data(activity_filepath)

    cricket = [[] for _ in range(7)]
    chess = [[] for _ in range(7)]
    mathematics = [[] for _ in range(7)]
    science = [[] for _ in range(7)]
    computers = [[] for _ in range(7)]
    blackrose = [[] for _ in range(7)]

    for activity in sports + talent + academics:
        eligible_participants = []
        activity_type = activity.activity_name.lower()

        for participant in participants:
            if isinstance(activity, SportsTournament) and isinstance(participant, Athlete):
                if participant.is_eligible():
                    eligible_participants.append(participant)
            elif isinstance(activity, TalentShow) and isinstance(participant, Artist):
                if participant.is_eligible():
                    eligible_participants.append(participant)
            elif isinstance(activity, AcademicCompetition) and isinstance(participant, Scholar):
                if participant.is_eligible():
                    eligible_participants.append(participant)

        activity.set_values({"participants": eligible_participants})

        if len(eligible_participants) < 2:
            activity.set_values({"is_active": False})

        grade_index = activity.get_grade_level() - 6
        if grade_index < 0 or grade_index > 6:
            continue

        if activity_type == "cricket":
            cricket[grade_index].append(activity)
        elif activity_type == "chess":
            chess[grade_index].append(activity)
        elif activity_type == "mathematics":
            mathematics[grade_index].append(activity)
        elif activity_type == "science":
            science[grade_index].append(activity)
        elif activity_type == "computers":
            computers[grade_index].append(activity)
        elif activity_type == "blackrose":
            blackrose[grade_index].append(activity)

    return cricket, chess, mathematics, science, computers, blackrose


def winners(activities: Tuple) -> None:
    cricket, chess, mathematics, science, computers, blackrose = activities

    for grade in range(6, 13):
        grade_index = grade - 6
        grade_results = []

        def find_winner(activity_list):
            for activity in activity_list:
                if activity.get_is_active():
                    winner = activity.determine_winner()
                    if winner != -1:
                        return f"{winner.get_class_assigned()}, {winner.idi}"
            return "NA"

        grade_results.append(find_winner(cricket[grade_index]))
        grade_results.append(find_winner(chess[grade_index]))
        grade_results.append(find_winner(mathematics[grade_index]))
        grade_results.append(find_winner(science[grade_index]))
        grade_results.append(find_winner(computers[grade_index]))
        grade_results.append(find_winner(blackrose[grade_index]))

        with open(f"grade{grade}.csv", "w", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(["cricket", "chess", "mathematics", "science", "computers", "blackrose"])
            writer.writerow(grade_results)
